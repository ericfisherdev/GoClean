...
func parseComplexGoCode(t *testing.T, code string) *scanner.GoASTInfo {
	...
	ast.Inspect(file, func(n ast.Node) bool {
		...
		case *ast.FuncDecl:
			if node.Name != nil {
				...
				// Extract parameters
				var parameters []scanner.ParameterInfo
				if node.Type.Params != nil {
					for _, param := range node.Type.Params.List {
						for _, name := range param.Names {
							parameters = append(parameters, scanner.ParameterInfo{
								Name: name.Name,
